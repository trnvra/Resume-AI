...
async function generateResumePdf({ resume, selfDescription, jobDescription }) {
    ...
    const jd = jobDescription.toLowerCase()
    if (jd.includes("frontend") || jd.includes("react.js") && !jd.includes("node")) resumeType = "frontend"
    else if (jd.includes("backend") || jd.includes("node.js") && !jd.includes("react")) resumeType = "backend"
    else if (jd.includes("react native") || jd.includes("mobile")) resumeType = "reactnative"
    else if (jd.includes("full stack") || jd.includes("fullstack")) resumeType = "fullstack"

    const pdfBuffer = await generatePdfFromHtml(mockResumes[resumeType])
    return pdfBuffer
}
